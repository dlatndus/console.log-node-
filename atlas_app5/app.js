...
function connectDB() {
	// 데이터베이스 연결 정보
	var databaseUrl = 'mongodb+srv://sooyeon:alflarhkgkrrh1@sooyeon.giri3il.mongodb.net/myuserdb';
...
function createUserSchema() {
	...
	UserSchema.virtual('password').set(function (password) {
		...
		console.log("virtual pass의 get 호출");
	...
	// 인증 메소드 - 입력된 비밀번호와 비교(true/false리틴) 
	UserSchema.method('authenticate', function (plainText, inSalt, hashed_password) {
		if (inSalt) {

			console.log('authenticate: %s -> %s:%s', plainText, this.encryptPassword(plainText, inSalt), hashed_password);
			return this.encryptPassword(plainText, inSalt) === hashed_password;
		}
		else {
			console.log('authenticate: %s -> %s:%s', plainText, this.encryptPassword(plainText), this.hashed_password);
	...
	// 저장 시의 트리거함수 정의 (password 필드가 유효하지 않으면 에러 발생) 
	...
	if (database) {
		authUser(database, paramId, paramPassword, function (err, docs) {
			...
			if (docs) {
				...
				// res.write('<hr>');
				// res.write('<button class="btn btn-info"><a href="/adduser.html" >사용자추가</a></button>');
				// res.write('<button class="btn btn-info"><a href="/listuser.html" >사용자리스트</a></button>');
				...
				// res.write('<hr>');
				// res.write('<div><a href="/login.html" class="btn btn-info">다시로그인</a></div>');
	...
	if (database) {
		addUser(database, paramId, paramPassword, paramName, function (err, addedUser) {
			...
			if (addedUser) {
				...
				// res.write('<hr>');
				// res.write('<div><a href="/adduser.html" class="btn btn-info">사용자추가</a></div>');
				// res.write('<div><a href="/listuser.html" class="btn btn-info">사용자리스트</a></div>');
				// res.write('<div><a href="/" class="btn btn-info">메인페이지</a></div>');
				// res.end();
	...
	if (database) {
		...
		try {
			...
			if (results) {  // 결과 객체 있으면 리스트 전송
				...
				res.write('</ul></div>');
				res.write('<br>');
				res.write('<button><a href="/adduser.html" class="btn btn-info">사용자추가</a></button>');
				res.write('<button><a href="/login.html" class="btn btn-info">로그인</a></button>');
				res.end();
			} else {  // 결과 객체가 없으면 실패 응답 전송
				res.writeHead('200', { 'Content-Type': 'text/html;charset=utf8' });
